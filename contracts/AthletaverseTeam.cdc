import AthletaverseAthlete from "./AthletaverseAthlete.cdc"
...
pub contract AthletaverseTeam {
    ...
    pub resource Asset: TeamPublic, TeamManager {
        ...
        pub var roster: @{UInt64: AthletaverseAthlete.NFT}
        ...
        // addAthleteToTeam adds an Athlete NFT to the Team roster
        //
        // - allows the Athlete to participate in Team activities
        //
        pub fun addAthleteToTeam(athlete: @AthletaverseAthlete.NFT) {
            
            // get the ID from the Athlete NFT
            let ID = athlete.id

            // add the Athlete to the roster 
            // (remove and destroy the old Athlete - if any)
            //
            let oldAthlete <- self.roster[ID] <- athlete
            destroy oldAthlete
            
            emit AthleteAddedToTeam(teamID: self.id, athleteID: ID)
        }

        // removeAthleteFromTeam removes an Athlete NFT from the Team roster
        // and returns it to the caller
        //
        // - the Athlete can no longer participate in Team activities
        pub fun removeAthleteFromTeam(_ ID: UInt64) {
            pre {
                self.roster[ID] != nil: "There is no Athlete with this ID on the roster"
            }

            // remove the Athlete from the roster
            let athlete <- self.roster.remove(key: ID)
